 ...
 * @param CachedMapIDs
 * @type number[]
 * @desc Array of map IDs from which cache will be saved.
 * @default []
 *
 * @help
 * ============================================================================
...
(function() {
  ...
  const cachedMapIDs = _PARAMS['CachedMapIDs'].map(Number);
  
  let cachedCollisionMaps = {}; // Global cache for collision maps
  ...
  Game_Map.prototype.loadCollisionmap = function(collisionMap) {
    if (collisionMap) {
      ...
      if (cachedMapIDs.includes(mapId)) {
	      if (!cachedCollisionMaps[mapId]) {
		cachedCollisionMaps[mapId] = ImageManager.loadBitmap(_CMFOLDER, collisionMap);
	      }
	      ColliderManager.collisionMap.bitmap = cachedCollisionMaps[mapId];
      }else ColliderManager.setCollisionMap(collisionMap);
      this._hasCM = true;
    } else {
